...
class Sensor:
    ...
    def min_Power_data(self):
        return 5 * self.datamin / 1000

    def max_Power_data(self):
        return 5 * self.datamax / 1000

    def total_min_power(self):
        return self.wirpmin + (5 * self.datamin * 1e-9)

    def total_max_power(self):
        return self.wirpmax + (5 * self.datamax * 1e-9)
...
s3 = Sensor('Battery Monitor', 5, 8, 41.25, 57.75, 2.475e-07, 2.5e-07, 3, 0)
s4 = Sensor('Temprature', 6, 12, 100, 1750, 5e-07, 1e-06, 4, 6.6e-10)
s5 = Sensor('Accelerometer', 8, 14, 6600, 8250, 2.53e-05, 4.4e-05, 5, 3.6e-8)
...
s8 = Sensor('Imager(VGA,RGB', 8, 10, 100000000, 1750000000, 4.17e-01, 0.2525, 8, 8.8e-4)
s9 = Sensor('Imager (MP4 compressed)', 8, 10, 17500000, 17830000, 1.70E-01, 2.53E-01, 9, 8.8e-4)
